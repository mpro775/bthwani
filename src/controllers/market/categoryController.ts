...
export const createCategory = async (req: Request, res: Response) => {
  try {
    ...
    const category = new ProductCategory({ name, image, categoryId, parentCategory });
    await category.save();
...
export const getAllCategories = async (_: Request, res: Response) => {
  const categories = await ProductCategory.find().populate("parentCategory", "name");
  res.json(categories);
};
...
export const updateCategory = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const updated = await ProductCategory.findByIdAndUpdate(id, req.body, { new: true });
    if (!updated) {
