...
export const getFollowers = async (req: Request, res: Response) => {
  try {
    const user = await User.findById(req.params.id).populate("followers", "fullName profileImage");
    if (!user) {
res.status(404).json({ message: "User not found" });
      return;
    } 
...
export const getFollowing = async (req: Request, res: Response) => {
  try {
    const user = await User.findById(req.params.id).populate("following", "fullName profileImage");
    if (!user) {
res.status(404).json({ message: "User not found" });
      return;
    } 
