...
import DeliveryProductSubCategory from '../../models/delivry/DeliveryProductSubCategory';
...
export const update = async (req: Request, res: Response) => {
  try {
    const data = await DeliveryProductSubCategory.findByIdAndUpdate(req.params.id, req.body, { new: true });
    res.json(data);
  } catch (error: any) {
    res.status(500).json({ message: error.message });
  }
